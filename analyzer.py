...
stats = {
    ...
    "pros_count" : int(opinions.pros.astype(bool).replace(False,None).count()), #lub opinions.pros.astype(bool).sum()
    "cons_count" : int(opinions.cons.astype(bool).replace(False,None).count()),
...
print(f'''Dla produktu o kodzie {product_code} 
      ...
      Średnia ocena produktu wynosi {stats["avarage_score"]:.2f}.''')
